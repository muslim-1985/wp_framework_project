<?php
/**
 * solid walker nav menu extends wp walker nav menu
 *
 * @package bootstrap-basic
 */
if (!class_exists('Solid_Navwalker')) {
    class Solid_Navwalker extends Walker_Nav_Menu {
        ...
        public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
            ...
            if (is_object($args) && $args->has_children) {
                //$classes[] = 'dropdown';
                $li_attributes .= ' data-hover="dropdown" data-dropdown="dropdown"';
                $li_class .= ' dropdown';
            }
            $classes[] = 'menu-item-' . $item->ID;
            //If we are on the current page, add the active class to that menu item.
            $classes[] = ($item->current) ? 'active' : '';
            //If $item->title == 'logo', add class visible-desktop to that menu item
            $classes[] = ($item->title == 'Logo') ? 'visible-desktop' : '';
            ...
            $attributes .= ($item->title == 'Logo') ? ' class = "brand"' : '';
